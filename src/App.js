...
const costs = {
  ...
  meal: "2514.5",
  health: "1308.40",
  ufees: [409.77, 819.56, 1229.31, 1683.5],
  gfees: [326.67, 653.34, 1522.5],
};
...
const costsDLP = {
  ...
  gi: 739.17,
  go: 2473.68,
  ufees: [72.42, 144.84, 217.26, 289.68],
  gfees: [72.42, 144.84, 217.26],
};
      ...
      {(show) =>
        ...
        ((props) => (
          <div className='no-float' style={props}>
            <Hourscontainer level={level} change={change} credit={credit} />
          </div>
        ))
      }
    </Transition>
      ...
      {showCredit ? (
        <div className='no-float'>
          <div className='input-group-hours'>
            <input type='radio' id='three-hours' value="1" name='hours' onChange={props.change} defaultChecked></input>
            <label htmlFor='three-hours' className='number-font'>
              3
            </label>
          </div>
          <div className='input-group-hours'>
            <input type='radio' id='six-hours' value="2" name='hours' onChange={props.change}></input>
            <label htmlFor='six-hours' className='number-font'>
              6
            </label>
          </div>
          {undergradCheck && (
            <div className='input-group-hours'>
              ...
              <label htmlFor='nine-hours' className='number-font'>
                9
              </label>
            </div>
          )}
...
function LevelTwo(props) {
  ...
  return (
    <Transition
      config={{ duration: 300 }}
      items={show}
      from={{ maxHeight: 0, overflow: "hidden", padding: "0px 0px" }}
      enter={isMobile ? { maxHeight: 482, padding: "10px 0px" } : { maxHeight: 320, padding: "10px 0px" }}
      leave={{ maxHeight: 0, padding: "0px 0px" }}
    >
      {(show) =>
        show &&
        ((props) => (
...
class Total extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.props.creditCost !== prevProps.creditCost ||
      ...
      const previous =
        Number(prevProps.creditCost * prevProps.multiplier) +
        Number(prevProps.living) +
        Number(prevProps.car) +
        Number(prevProps.meal) +
        Number(prevProps.health) +
        prevProps.fees;
      const current =
        Number(this.props.creditCost * this.props.multiplier) +
        Number(this.props.living) +
        Number(this.props.car) +
        Number(this.props.meal) +
        Number(this.props.health) +
        this.props.fees;
    ...
    return (
      <div className='total-container' aria-live='polite'>
        ...
        <p className='total-cost'>
          $<Animatedtotal totalBefore={this.state.previousTotal} totalAfter={this.state.currentTotal} />
        </p>
        <p className='disclaimer'>
          *This total is an estimated cost paid to WCU. This estimation does not include program-specific fees, graduate tuition differentials, or
...
class App extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.multiplier == 2 && this.state.level === "grad" && this.state.level !== prevState.level && prevState.level !== "") {
  ...
  levelChanges(a, b, c) {
    if (this.state.credit === "part-time") {
      if (this.state.level === "undergrad" && this.state.multiplier == 3) {
        this.setState({ multiplier: 2, fees: c[1] });
      } else if (this.state.level === "") {
        this.setState({ multiplier: 1, fees: c[0] });
      } else {
        this.setState((state) => ({fees: c[Number(state.multiplier)] }));
      }
    }
    if (this.state.credit === "full-time") {
      if (this.state.level === "undergrad") {
        this.setState({multiplier:3,fees: c[2] });
      } else {
        this.setState({multiplier:4,fees: c[3] });
      }
    }
    this.state.resident === "in-state"
      ? this.setState({ creditCostInitial: a })
      : this.state.resident === "out-of-state"
      ? this.setState({ creditCostInitial: b })
      : this.setState({ creditCostInitial: 0 });
  }
  handleLevel(e) {
    this.setState({ [e.target.name]: e.target.value });
    const costVar = this.state.distance ? costs : costsDLP;
    if (e.target.value === "undergrad") {
      this.levelChanges(costVar.ui, costVar.uo, costVar.ufees);
    } else {
      this.levelChanges(costVar.gi, costVar.go, costVar.gfees);
    }
  }
  residentChanges(a, b) {
    this.state.level === "undergrad"
      ? this.setState({ creditCostInitial: a })
      : this.state.level === "grad"
      ? this.setState({ creditCostInitial: b })
      : this.setState({ creditCostInitial: 0 });
  }
  handleResident(e) {
  ...
  handleCredit(e) {
    ...
    if (e.target.id === "full-time") {
      ...
      this.state.level === "undergrad"
        ? this.setState({ fees: costVar.ufees[e.target.value - 1] })
        : this.state.level === "grad"
        ? this.setState({ fees: costVar.gfees[e.target.value - 1] })
        : this.setState({ fees: 0 });
    } else {
      this.setState({ living: 0, health: 0, multiplier: 1 });
      this.state.level === "undergrad"
        ? this.setState({ fees: costVar.ufees[0] })
        : this.state.level === "grad"
        ? this.setState({ fees: costVar.gfees[0] })
        : this.setState({ fees: 0 });
    }
  }
  handleHours(e) {
    ...
    const costVar = this.state.distance ? costs : costsDLP;
    this.state.level === "undergrad"
      ? this.setState({ fees: costVar.ufees[Number(e.target.value) - 1] })
      : this.state.level === "grad"
      ? this.setState({ fees: costVar.gfees[Number(e.target.value) - 1] })
      : this.setState({ fees: 0 });
    if (e.target.id === "three-hours") {
  ...
  distanceChanges(a) {
    ...
    if (this.state.level !== "" && this.state.credit !== "") {
      if (this.state.level === "undergrad") {
        this.state.resident === "in-state"
          ? this.setState({ creditCostInitial: a.ui, fees: a.ufees[multiplier - 1] })
          : this.state.resident === "out-of-state"
          ? this.setState({ creditCostInitial: a.uo, fees: a.ufees[multiplier - 1] })
          : this.setState({ creditCostInitial: 0, fees: a.ufees[multiplier - 1] });
      } else {
        this.state.resident === "in-state"
          ? this.setState({ creditCostInitial: a.gi, fees: a.gfees[multiplier - 1] })
          : this.state.resident === "out-of-state"
          ? this.setState({ creditCostInitial: a.go, fees: a.gfees[multiplier - 1] })
          : this.setState({ creditCostInitial: 0, fees: a.gfees[multiplier - 1] });
      }
    }
  }
  handleDistance(e) {
        ...
        <form id='cost-calculator-form'>
          <div class='section-one'>
            ...
            <div className='input-container' role='group' aria-labelledby='credit'>
              ...
              <InputGroup id={"full-time"} value={this.state.level === "undergrad" ? "4" : "3"} label={"Full-Time"} name={"credit"} change={this.handleCredit} />
          ...
          <LevelTwo
            ...
            expenses={this.handleOtherExpenses}
            distance={this.state.distance}
          />
        </form>
        <Total
          creditCost={this.state.creditCostInitial}
          multiplier={this.state.multiplier}
          distance={this.state.distance}
